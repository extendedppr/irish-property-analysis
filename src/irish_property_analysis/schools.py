    ...
    def get_near(self, lat, lng, radius_km=1):
        ...
        for idx, data in enumerate([self.primary, self.secondary]):
            ...
            if idx == 1:
                ...
                data['School Latitude'] = pd.to_numeric(data['School Latitude'], errors='coerce')
                data['School Longitude'] = pd.to_numeric(data['School Longitude'], errors='coerce')
            ...
            final_data.extend(data.loc[mask].assign(distance_km=distances[mask]).sort_values(by='distance_km').reset_index(drop=True).to_dict(orient='records'))
