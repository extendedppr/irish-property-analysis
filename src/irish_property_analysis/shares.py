...
class ShareDB:
    ...
    def filter(
        ...
        query = ShareObject.select()

        for field, value in filters.items():
            if value is not None:
                field_name = getattr(ShareObject, field)
                if partial:
        ...
        if address:
            ...
            if partial:
                query = query.where(ShareObject.searchable_address.contains(address))
            else:
                query = query.where(ShareObject.searchable_address == address)

        if address_substrs:
            for address_substr in address_substrs:
                if partial:
                    query = query.where(
                        ShareObject.searchable_address.contains(address_substr)
                    )
                else:
                    query = query.where(
                        ShareObject.searchable_address == address_substr
                    )
        ...
        if exclude_address_substrs:
            for exclude_address_substr in exclude_address_substrs:
                if partial:
                    ...
                    query = query.where(
                        ~(ShareObject.searchable_address == exclude_address_substr)
                    )
